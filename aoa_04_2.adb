...
procedure AoA_04_2 is
   ...
   package Coord_Lists is new Ada.Containers.Vectors (Index_Type => Positive, Element_Type => Coord);

   Input : Ada.Text_IO.File_Type;
   Map   : Map_Layout := (others => (others => '.') );
   Count : Natural;
   Sum   : Natural;
   Free  : Coord_Lists.Vector;
   Loc   : Coord;
   Total : Natural := 0;
begin -- AoA_04_2
         ...
         All_Cols : for Col in Map (Row)'First + 1 .. Map (Row)'Last - 1 loop
            if Map (Row) (Col) = '@' then -- Count the neighbors of (Row, Col)
               ...
               if Count < 4 then -- This roll can be accessed
                  Sum := Sum + 1;
                  Free.Append (New_Item => (Row => Row, Col => Col) );
               end if;
            end if;
         end loop All_Cols;
      end loop All_Rows;

      exit All_Passes when Sum = 0;

      Total := Total + Sum;

      Remove : for I in 1 .. Free.Last_Index loop
         Loc := Free.Element (I);
         Map (Loc.Row) (Loc.Col) := '.';
      end loop Remove;
   end loop All_Passes;

   Ada.Text_IO.Put_Line (Item => Total'Image);
end AoA_04_2;
